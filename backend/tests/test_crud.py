def test_create_task(client):
    response = client.post("/tasks", json={"title": "Test task", "description": "Test description", "completed": "False"})
...
def test_read_nonexistent_task(client):
    response = client.get("/tasks/999")  # Assuming 999 does not exist
...
def test_delete_nonexistent_task(client):
    response = client.delete("/tasks/999")  # Assuming 999 does not exist
    assert response.status_code == 404
    assert response.json() == {"detail": "task not found"}
...
def test_read_tasks_after_deletion(client):
    task_id = test_create_task(client)  # Create a task_list_item to delete it
    client.delete(f"/tasks/{task_id}")  # Delete the created task_list_item
    response = client.get("/tasks/")
    assert response.status_code == 200
    tasks = response.json()
